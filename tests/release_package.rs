...
fn visible_text_joins_a_wrapped_schema_refusal() {
    let recorded = "application metadata schema\r\n\u{1b}[0m99 is newer than supported schema 13.";
    let visible = visible_text(recorded);
    assert!(
        visible.contains("application metadata schema 99 is newer than supported schema 13"),
        "{visible}"
    );
...
fn future_schema_gate_accepts_csi_glued_schema_version() {
    ...
    let recorded = "application metadata schema\u{1b}[8;1H99 is newer than supported schema 13.";
    let visible = visible_text(recorded);
    assert!(
...
fn future_schema_refusal_is_visible(visible: &str) -> bool {
    visible.contains("99 is newer than supported schema 13")
